---
name: qualidade-3-zerar
description: Zera uma pilha acumulada de avisos de lint (ESLint/Biome) — inclusive os que sobraram depois de qualidade-1-medir e qualidade-2-quebrar — em ondas rastreadas, sem quebrar nada. Use quando houver muitos warnings antigos e o projeto JÁ tenha lint configurado. Se não tiver configuração ainda, use antes configurar-lint (filosofia completa) ou qualidade-1-medir (só os três gates de tamanho/console/dados). Sugerir quando o build acusar dezenas de avisos.
---

# Qualidade 3 — Zerar

Executa, para o conjunto de avisos restantes, o prompt
`02-eslint-warning-burndown.md` do vibe-coding-toolkit (commit `13add21`,
copiado em `docs/prompts-toolkit/02-eslint-warning-burndown.md`). Terceiro e
último passo da sequência: `qualidade-1-medir` instala e mede,
`qualidade-2-quebrar` divide os arquivos grandes por responsabilidade, e
esta zera o que sobrou — `no-direct-console`, `no-direct-data-access`,
`complexity`, ou qualquer outra regra com pilha acumulada.

## Verificação antes de rodar

- **Sem `package.json`** no projeto, ou ESLint (ou Biome) não configurado →
  avise e pare. Sem lint configurado não existe pilha de avisos para zerar
  — use `configurar-lint` primeiro.

## Descobrir os comandos, nunca presumir

Leia o `scripts` do `package.json` e o lockfile presente (`npm`, `pnpm`,
`yarn` ou `bun`) para preencher `[LINT_COMMAND]`, `[TYPECHECK_COMMAND]`,
`[TEST_COMMAND]` e `[BUILD_COMMAND]` com os comandos reais do projeto —
**nunca presuma `npm run lint`**. Alguns wrappers de CI resumem a saída do
lint em vez de mostrar cada linha; se isso acontecer, rode com a flag ou
invocação direta que devolve a saída completa.

## Quando usar

Se o projeto ainda não tem uma configuração de ESLint decente para
começar, use antes `configurar-lint` (monta a config do zero, com
filosofia opinativa completa) ou `qualidade-1-medir` (o atalho — copia só
as três regras de tamanho/console/dados e mede). Esta skill serve para
depois, quando os avisos de uma configuração já existente acumularam e é
hora de zerá-los.

Se os avisos que você quer zerar são especificamente de **tamanho de
arquivo**, vá direto para `qualidade-2-quebrar`: quebrar um arquivo de 800
linhas é um tipo de trabalho diferente de corrigir 240 avisos de
`complexity`, e aquela skill é feita só para isso.

Use esta skill quando você tem uma pilha grande de warnings de lint para
...
grande. Ela foi generalizada a partir de uma queima de warnings real, na
...
rápido demais para confiar nelas. Depois, ele isola a decisão mais
...
mudança de configuração, não uma limpeza de código, para nunca ser
...
disjuntos, despachadas em paralelo (use a skill `ondas-paralelas` para a
mecânica de despacho) — porque um subagente só enxerga o próprio
briefing, nunca a conversa de planejamento inteira; por isso a lista de
pegadinhas precisa ser copiada literalmente em cada uma. E o checklist
final só é riscado rodando o comando de novo, nunca por suposição.
...
- **`[RULE_NAME]`** — a regra específica do ESLint (ou de outro linter)
  ...
  `complexity`, `quality/no-direct-console`. Se a queima cobre o conjunto
  inteiro de warnings em vez de uma regra só, diga isso explicitamente — o
  próprio prompt já prevê essa opção ("or the full warning set").
- **`[STACK/FRAMEWORK]`** — descrição curta da stack, para dar contexto ao
  agente.

Os outros quatro placeholders (`[LINT_COMMAND]`, `[TYPECHECK_COMMAND]`,
`[TEST_COMMAND]`, `[BUILD_COMMAND]`) vêm do `package.json`, não de
pergunta — ver "Descobrir os comandos" acima.
...
it. Use file-disjoint parallel wave dispatch (this project's `ondas-paralelas`
skill covers the grouping rule and dispatch mechanics).
...
`complexity` do ESLint para um valor mais rígido revelou 240 warnings no
projeto inteiro — 180 deles concentrados em seis arquivos legados com
condicionais muito aninhadas. Os comandos vieram do `package.json` real
(`[LINT_COMMAND]` → `npm run lint`, `[TYPECHECK_COMMAND]` → `npm run
typecheck`, `[TEST_COMMAND]` → `npm run test`, `[BUILD_COMMAND]` → `npm
run build`), e `[RULE_NAME]` → `complexity`.
...
resto como dívida técnica, ou afrouxar o limite da regra só para esses
seis arquivos (deixando explícito que isso é mudança de config, não
limpeza de código). Sem resposta, ele assume a opção A como padrão e
segue corrigindo o resto do projeto normalmente enquanto espera. Depois,
quebra o trabalho em ondas de arquivos disjuntos, cada uma com uma
pegadinha conhecida copiada no briefing (ex.: "essa extração não pode
mudar o tipo de retorno inferido da função exportada"), roda revisores
especialistas por onda, e só risca o checklist final depois de rodar
`[LINT_COMMAND]`, `[TYPECHECK_COMMAND]`, `[TEST_COMMAND]` e
`[BUILD_COMMAND]` de novo, de verdade.
...
- Combine o passo 3 com `ondas-paralelas` para ter paralelismo de
  verdade; sem isso, isso degrada para uma tarefa por vez.
